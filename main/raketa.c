...
void app_main(void){
    ...
    ESP_ERROR_CHECK(sx127x_tx_set_pa_config(SX127x_PA_PIN_BOOST,
                current_power_level, &device));
...
void loraTx(sx127x *device) {
    ...
    ESP_ERROR_CHECK(sx127x_lora_tx_set_for_transmission(data, sizeof(data),
                device));
    ESP_ERROR_CHECK(sx127x_set_opmod(SX127x_MODE_TX, SX127x_MODULATION_LORA,
                device));
...
void setup_gpio_interrupts(gpio_num_t gpio, sx127x *device,
  ...
  ESP_ERROR_CHECK(gpio_isr_handler_add(gpio, handle_interrupt_fromisr,
              (void *) device));
}
